...
func (c *Controller) queryRegionList(context *gin.Context) {
	...
	if !config.GetConfig().Hk4e.ForwardModeEnable {
		...
		var err error = nil
		regionListBase64, err = httpclient.GetRaw(config.GetConfig().Hk4e.ForwardRegionUrl)
		if err != nil {
			_, _ = context.Writer.WriteString("500")
			return
		}
...
func (c *Controller) queryCurRegion(context *gin.Context) {
	...
	if !config.GetConfig().Hk4e.ForwardModeEnable {
		...
		regionCurrJson, err := httpclient.GetRaw(config.GetConfig().Hk4e.ForwardDispatchUrl)
		if err != nil {
			rspError()
			return
		}
		...
		regionCurr.RegionInfo = queryCurrRegionHttpRsp.RegionInfo
